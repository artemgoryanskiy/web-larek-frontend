import { ApiCartItem, ApiOrder, ApiProduct } from './api';
...
export class CartTransformer {
	...
	static itemFromApi(
		...
		return {
			...
			quantity: defaultQUnatity,
			price: (product.price ?? 0) * defaultQUnatity,
		};
	}
	 ...
	 */
	static updateProductsInCartStatus(
		products: IProduct[],
		cartItems: ICartItem[]
	): IProduct[] {
...
export class OrderTransformer {
	...
	static mergeFormData(
		existingData: IOrderFormData,
		newData: Partial<IOrderFormData>
	): IOrderFormData {
		return {
			...existingData,
			...newData,
